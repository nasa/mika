...
class test_FMEA(unittest.TestCase):
    ...
    def test_FMEA_integration(self):
        ...
        self.test_class.group_docs_with_meta(self.test_grouping_col)
        ...
        self.test_class.post_process_fmea(phase_name='', id_name='test', max_words=1)
        fmea_1 = self.test_class.fmea_df.copy()
        self.setUp()
        self.test_class.load_data(filepath=self.test_filename, formatted=False, text_col=self.test_text_col, id_col=self.test_id_col, label_col=None)
        self.test_class.load_model(self.test_model_checkpoint)
        self.test_class.predict()
        self.test_class.build_fmea(severity_func, group_by='meta', year_col='date', group_by_kwargs={'grouping_col':self.test_grouping_col}, post_process_kwargs={'phase_name':'', 'id_name':'test', 'max_words':1}, save=True)
        os.remove('fmea.csv')
