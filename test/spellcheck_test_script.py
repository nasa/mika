...
no_spellcheck = Topic_Model_plus(list_of_attributes=list_of_attributes, document_id_col=document_id_col, csv_file=csv_file_name, name=name, combine_cols=True,quot_correction=True,spellcheck=False,segmentation=False)
no_spellcheck.prepare_data()
no_spellcheck.preprocess_data()
no_spellcheck.hlda()

print('Generating topic model object with spellchecker...')
spellcheck = Topic_Model_plus(list_of_attributes=list_of_attributes, document_id_col=document_id_col, csv_file=csv_file_name, name=name, combine_cols=True,quot_correction=True,spellcheck=True,segmentation=False)
spellcheck.prepare_data()
spellcheck.preprocess_data()
spellcheck.hlda()

print('Generating topic model object with segmentation...')
segmentation = Topic_Model_plus(list_of_attributes=list_of_attributes, document_id_col=document_id_col, csv_file=csv_file_name, name=name, combine_cols=True,quot_correction=True,spellcheck=False,segmentation=True)
segmentation.prepare_data()
segmentation.preprocess_data()
segmentation.hlda()

print('Generating topic model object with both...')
both = Topic_Model_plus(list_of_attributes=list_of_attributes, document_id_col=document_id_col, csv_file=csv_file_name, name=name, combine_cols=True,quot_correction=True,spellcheck=True,segmentation=True)
both.prepare_data()
both.preprocess_data()
both.hlda()
